...
A Chrome/Edge extension that removes Gemini AI star watermarks from images using MI-GAN deep learning inpainting.

## Features

- 🎯 **Automatic Detection**: Uses template matching to find the 4-pointed star watermark in the bottom-right corner
- 🧠 **AI-Powered**: Uses MI-GAN (~30MB) for high-quality inpainting
- 🖱️ **Right-Click Menu**: Right-click any image and select "Remove Gemini Watermark"
...
### Remove Watermark from Single Image:
1. Right-click on any image with a Gemini watermark
2. Select "Remove Gemini Watermark"
...
3. All images with detected watermarks will be processed

## How It Works

1. **Detection**: Uses template matching (ported from Python/OpenCV) to locate the star watermark
2. **Masking**: Creates a precise star-shaped mask around the detected watermark
...
- Works best on images where the watermark is clearly visible
- The star watermark should be in the bottom-right area
